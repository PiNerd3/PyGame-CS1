...
def _round(number):
    _f = math.floor(number)
    return _f if _f <= number < _f + 0.5 else math.ceil(number)
...
mvmt_prompt = times.render("Try pressing Ctrl, left arrow key, or right arrow key!", True, WHITE, BLACK)
...
_dx_orig, rot_orig = _round(2), 0.25
...
while True:
    ...
    windowSurface.blit(img, imgRect.topleft)
    ...
    pygame.draw.rect(windowSurface, BLACK, pygame.Rect(
    ...
    if ctrl_pressed:
        ...
        cirMode = 'fd'
    #Quitting on 'q'
    if q_quit.lower() in {'q', 'quit',}.union({'t', 'true' , 'y', 'yes'}):
        if keys[K_q]:
            pygame.quit()
            sys.exit()
